...
Replace `FootballSupporters` / `football_supporters` with your real app/module names. The Sinatra base
...
(e.g. `FootballSupporters::MediaClient`) — see `.claude/external-service-integration.md`.
...
| Strategy                    | When                                                      |
|-----------------------------|-----------------------------------------------------------|
| Stub the **client class**   | Unit specs of code that calls your `FootballSupporters::MediaClient`; fast, no HTTP layer involved |
...
media = instance_double(FootballSupporters::MediaClient,
                        get_asset: { duration: 120.5, max_resolution: "1080p" })
allow(FootballSupporters::MediaClient).to receive(:new).and_return(media)

# Or replay a recorded interaction (integration-level):
it "fetches the asset", :vcr do   # uses cassette named after the example
  expect(FootballSupporters::MediaClient.new.get_asset("asset_123")).to include(status: "ready")
end
```
