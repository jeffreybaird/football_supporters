  ...
  class Create
    ...
    def call(account:, actor:, attrs:)
      ...
      if note.save                                                   # false + note.errors on invalid
        FootballSupporters::Audit.record("note.created", resource: note, actor: actor)  # side effects: see §5
        Success(note)
      else
...
# FootballSupporters::Result.err(:plan_limit_reached, { limit: 100, current: 100 })
...
FootballSupporters::Audit.record("note.created", resource: note, actor: actor)
```
...
note.soft_delete
FootballSupporters::Audit.record("note.deleted", resource: note)
```
...
def call(...)
  ...
  FootballSupporters::Audit.record("note.created", resource: note, actor:)              # inline: cheap, durable (§2)
  FootballSupporters::WebhookJob.perform_async(note.account_id, "note.created", note.to_hash)  # enqueued (Sidekiq)
  FootballSupporters::Cache.delete("account:#{note.account_id}:note_count")             # bust the cache key (if Redis cache)
...
| Side effect | How |
|-------------|-----|
| Audit log | `FootballSupporters::Audit.record(…)` inline after commit (§2) — cheap, must not be lost |
| Webhook dispatch | `FootballSupporters::WebhookJob.perform_async(account_id, event, payload)` (Sidekiq) |
| Notification email | enqueue a Sidekiq job that sends via `Mail`/`Pony` — there is no ActionMailer |
| Cache invalidation | `FootballSupporters::Cache.delete(…)` over Redis — only if a cache is configured (`app/cache.rb`) |
...
```ruby
FootballSupporters::Flags.enabled?(:new_editor, actor: Current.user)   # per-actor / percentage
FootballSupporters::Flags.enable_percentage(:new_editor, 25)           # gradual rollout
FootballSupporters::Flags.enable_for_account(:beta_export, account)    # by account (group)
```

- ✅ `return Failure([:feature_not_enabled]) unless FootballSupporters::Flags.enabled?(:export, account:)`
...
FootballSupporters::WebhookJob.perform_async(account.id, event, payload)
```
...
- [ ] Plan-tier / rollout behavior is gated behind a `flags` entry (`FootballSupporters::Flags`), not scattered plan checks
- [ ] Background jobs (if any) include `account_id` in args; idempotent + unique; repopulate `Current` in `perform`
- [ ] `FootballSupporters::ExportAccountData` updated to include the new tenant-scoped table
