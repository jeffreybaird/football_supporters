...
Every UI addition or modification in `FootballSupporters` MUST fully comply with WCAG 2.1 AA. This is
...
- Every `<img>` MUST have a meaningful `alt` (or `alt=""` if purely decorative).
  - **Sinatra/ERB:** `<img src="/img/logo.png" alt="FootballSupporters">` — never omit `alt`.
