...
module FootballSupporters
  module Buffers
    module AnalyticsBuffer
      extend FootballSupporters::Buffer
      KEY = "buffer:analytics".freeze

      def self.write(_key, value)
        FootballSupporters.redis.rpush(KEY, value.to_json)
        :ok
      end

      def self.flush
        rows = []
        while (raw = FootballSupporters.redis.lpop(KEY))
...
def record_view(resource)
  FootballSupporters::Buffers::AnalyticsBuffer.write(:resource_view, {
...
in-process buffer stops sharing state — move it to Redis. Keep the `FootballSupporters::Buffer`
...
`FootballSupporters::Buffers::AnalyticsBuffer.flush`; cron granularity is one minute, so for a
...
```ruby
FootballSupporters.redis.hincrby("views:account:#{Current.account.id}", resource.id, 1)  # hot path
...
|---|---|---|
| **Cache** | tenant resolution (slug→account), theme/branding, layout config, catalog metadata, feature flags, entitlement status | `FootballSupporters::Cache.fetch` with TTL + invalidation on write |
...
module FootballSupporters
  module Cache
    def self.fetch(key, ttl: 300)
      if (raw = FootballSupporters.redis.get(key))
        return JSON.parse(raw, symbolize_names: true)
      end
      value = yield
      FootballSupporters.redis.set(key, value.to_json, ex: ttl)  # store JSON-safe data, not live objects
      value
    end

    def self.delete(key) = FootballSupporters.redis.del(key)
  end
end
```
...
def account_theme
  FootballSupporters::Cache.fetch("account:#{Current.account.id}:theme", ttl: 300) do
...
def note_card_html(note)
  FootballSupporters::Cache.fetch("note:#{note.id}:card:#{note.updated_at.to_i}", ttl: 3600) do
...
FootballSupporters::Cache.delete("account:#{theme.account_id}:theme")

# or, so no call site can forget it — Sequel fires after_commit post-commit:
class Theme < Sequel::Model
  def after_commit
    super
    FootballSupporters::Cache.delete("account:#{account_id}:theme")
  end
end
```
...
get "/accounts/:id/notes/stream" do
  ...
  stream(:keep_open) do |out|
    sub = FootballSupporters.redis_subscriber          # a DEDICATED redis connection for pub/sub
...
FootballSupporters.redis.publish("account:#{account_id}:notes",
...
def publish_note(attrs)
  ...
  FootballSupporters.redis.publish("account:#{Current.account.id}:notes",
                      { id: note.id, action: "created" }.to_json)
  note
end
...
FootballSupporters::Cache.fetch("account:#{Current.account.id}:layout") { ... }
...
- [ ] Frequently-read/rarely-written data goes through `FootballSupporters::Cache` (Redis) with invalidation on write
