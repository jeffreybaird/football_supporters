...
Replace `FootballSupporters` / `football_supporters` with your real app name. The Sinatra base class is a
...
1. **Every business mutation gets a manual span.** If a service object creates,
   ...
   `football_supporters.<context>.<operation>`. No library knows your business operations.
...
OpenTelemetry::SDK.configure do |c|
  c.service_name = "football_supporters"
...
OTEL_SERVICE_NAME=football_supporters          # overrides service_name above if set
```
...
tracer.in_span("football_supporters.billing.cancel_subscription") { ... }

# ❌ Manual span wrapping a plain Sequel query — -sequel already covers it (when active)
tracer.in_span("football_supporters.notes.get") { Note[id] }
```
...
module Notes
  class Create
    TRACER = OpenTelemetry.tracer_provider.tracer("football_supporters.notes")

    def self.call(account:, params:)
      TRACER.in_span("football_supporters.notes.create") do |span|
        span.set_attribute("football_supporters.account.id", account.id)
...
span.set_attribute("football_supporters.invoice.id", invoice.id)
...
```ruby
TRACER.in_span("football_supporters.payments.charge") do |span|
  charge!
...
```
football_supporters.<context>.<operation>
```

Examples:
- `football_supporters.notes.create`
- `football_supporters.notes.update`
- `football_supporters.billing.create_checkout`
- `football_supporters.billing.cancel_subscription`
- `football_supporters.accounts.invite_member`
- `football_supporters.webhooks.dispatch`
- `football_supporters.imports.process_record`

External services use the service name:
- `football_supporters.storage_provider.create_upload_url`
- `football_supporters.payment_provider.create_subscription`

Jobs (only if you add Sidekiq, and only for an enrichment span):
- `football_supporters.job.webhook_event_processor`
- `football_supporters.job.analytics_aggregation`

---

## Span Attributes

### Business attributes — namespace under `football_supporters.*`

```ruby
span.set_attribute("football_supporters.account.id", account.id)  # tenant/org scope
span.set_attribute("football_supporters.user.id", user.id)        # user-initiated ops
```
...
```ruby
span.add_attributes(
  "football_supporters.service" => "storage_provider",
  "football_supporters.idempotency_key" => key
)
```
...
```ruby
span.add_attributes(
  "football_supporters.account.id" => args["account_id"],
  "messaging.system" => "sidekiq",   # semconv, usually auto-set
  "football_supporters.job.attempt" => attempt
)
```
...
For your own business attributes, namespace under `football_supporters.*`. Do **not** re-emit
a semconv attribute under a custom name, and do not duplicate one the
auto-instrumentation already sets.

```ruby
# ✅ semconv for HTTP/DB (auto), football_supporters.* for business attrs
span.set_attribute("football_supporters.account.id", account.id)

# ❌ duplicating a semconv attribute under a custom name
span.set_attribute("football_supporters.http_status", 200)  # http.response.status_code already set
```
...
```ruby
# ✅ ID only
span.set_attribute("football_supporters.user.id", user.id)

# ❌ PII in the telemetry pipeline
span.set_attribute("football_supporters.user.email", user.email)
```
...
class WebhookEventJob
  ...
  def perform(account_id, event_id)
    OpenTelemetry::Trace.current_span.set_attribute("football_supporters.account.id", account_id)
...
class App < Sinatra::Base
  before do
    ...
    OpenTelemetry::Trace.current_span.add_attributes(
      "football_supporters.account.id" => Current.account&.id,
      "football_supporters.user.id"    => Current.user&.id
    )
  end
...
- [ ] Business mutations wrapped in a `football_supporters.<context>.<operation>` span
- [ ] Span attributes include `football_supporters.account.id` for tenant/user-scoped ops
- [ ] External API calls span with `football_supporters.service` + idempotency key
